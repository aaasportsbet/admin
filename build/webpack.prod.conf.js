...
const webpackConfig = merge(baseWebpackConfig, {
  ...
  plugins: [
    ...
    new HtmlWebpackPlugin({
      ...
      // default sort mode uses toposort which cannot handle cyclic deps in certain
      // cases, and in webpack 4, chunk order in HTML doesn't matter anyway
    }),
...
if (config.build.generateAnalyzerReport || config.build.bundleAnalyzerReport) {
  ...
  if (config.build.bundleAnalyzerReport) {
    webpackConfig
      .plugins
      .push(new BundleAnalyzerPlugin({analyzerPort: 8080, generateStatsFile: false}))
  }

  if (config.build.generateAnalyzerReport) {
    webpackConfig
      .plugins
      .push(new BundleAnalyzerPlugin({analyzerMode: 'static', reportFilename: 'bundle-report.html', openAnalyzer: false}))
  }
}
